        ...
        {[0, 1, 2].map((i) => (
          ...
          >
            <span className={['inline-block text-xs px-2.5 py-1 rounded-full font-medium mb-3', BADGE_CLASSES[i]].join(' ')}>
              {t(`tokenization.pmCards.${i}.badge`)}
            </span>
            <h4 className="font-serif text-lg text-ink-dark mb-2 leading-snug">{t(`tokenization.pmCards.${i}.title`)}</h4>
            <p className="text-sm text-text-muted leading-relaxed">{t(`tokenization.pmCards.${i}.body`)}</p>
          </div>
        ))}
      </div>
    </div>
