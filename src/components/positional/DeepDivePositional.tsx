...
export default function DeepDivePositional() {
  ...
  return (
    <section className="my-10 border-t border-cream-200 pt-8">
      ...
      >
        <span className="font-serif text-xl">{t('positional.deepDive.toggle')}</span>
      ...
      {open && (
        <div className="mt-6 space-y-5 text-text-muted leading-relaxed">
          {[0, 1].map((i) => (
            <div key={i}>
              <h4 className="font-medium text-text mb-2">{t(`positional.deepDive.sections.${i}.title`)}</h4>
              <p className="text-sm">{t(`positional.deepDive.sections.${i}.body`)}</p>
            </div>
          ))}
          <div className="border-t border-cream-200 pt-5">
            <h4 className="font-medium text-text mb-3">{t('positional.deepDive.linksHeading')}</h4>
              ...
              {LINK_HREFS.map((href, i) => (
                <li key={i}>
                  <a href={href} target="_blank" rel="noopener noreferrer" className="text-ink hover:text-ember-dark underline-offset-2 hover:underline">
                    {t(`positional.deepDive.links.${i}.label`)}
                  </a>{' '}
                  <span className="text-text-muted/80">{t(`positional.deepDive.links.${i}.desc`)}</span>
                </li>
              ))}
            </ul>
          </div>
