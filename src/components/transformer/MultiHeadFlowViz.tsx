...
// 多头注意力流程可视化：N 个并行 head → Concat → W_O → 输出
// 把 2.4 大段文字改成图——每个 head 一张卡片，向下汇聚到 Concat 和 W_O

const HEADS = [
  { name: 'Head 1', focus: '指代关系', example: '"她"→"Lucy"', color: '#1E40AF', bg: '#DBEAFE' },
  { name: 'Head 2', focus: '语法成分', example: '"放"→主语+宾语', color: '#047857', bg: '#D1FAE5' },
  { name: 'Head 3', focus: '情感倾向', example: '"累" = 负面', color: '#B85A2A', bg: '#FFF7ED' },
  { name: 'Head 4', focus: '长距离关联', example: '"看到"→"猫"', color: '#A16207', bg: '#FEF3C7' },
];
...
export default function MultiHeadFlowViz() {
  ...
  const headH = 90;
  const headY = 30;
  const spacing = (W - headW * HEADS.length) / (HEADS.length + 1);
  const headXs = HEADS.map((_, i) => spacing + i * (headW + spacing));
    ...
    <div className="rounded-lg border-2 border-cream-200 bg-cream-50 p-4 overflow-x-auto">
      <svg viewBox={`0 0 ${W} ${H}`} className="w-full h-auto" style={{ minWidth: '600px' }}>
        ...
        {HEADS.map((_, i) => {
          ...
          const toX = concatX + ((i + 0.5) * concatW) / HEADS.length;
          const toY = concatY;
          return (
            <line
              key={`line-${i}`}
