...
const en = {
  ...
  transformer: {
    ...
    comp: {
      overview: {
        boxInput: 'Input',
        boxInputSub: "Each token's vector",
        boxAttn: 'Attention',
        boxAttnSub: 'Tokens communicate',
        boxAttnExample: '"她" looks at "Lucy"',
        boxFFN: 'FFN (feed-forward)',
        boxFFNSub: 'Each token digests',
        boxFFNExample: 'Extract relevant features',
        boxOutput: 'Output',
        boxOutputSub: 'For next layer',
        step1Label: 'Step 1',
        step1Title: 'Attention',
        step1Sub: 'Tokens communicate',
        step1Body: 'One token can\'t understand itself alone — "她" by itself is meaningless until you look back and learn whether 她 refers to Lucy or Mary. Attention lets each token actively look at other relevant tokens, fusing context into its own vector.',
        step1Hint: '※ Think of it as "a meeting": each token swaps information with the others and updates its own understanding.',
        step2Label: 'Step 2',
        step2Title: 'FFN (feed-forward)',
        step2Sub: 'Each token digests on its own',
        step2Body: 'After attention, each token has "heard" information from others. FFN lets each token take a private "thinking" pass — combining what it just heard with the knowledge it already carries, extracting higher-level features.',
        step2Hint: '※ Think of it as "going back to your desk after the meeting" — processing what you just heard.',
        stackHint: '🔁 These two operations stack — large LLMs have dozens to over 100 layers, each one being "Attention → FFN", abstracting more complex meaning each layer. Real engineering also wraps "engineering plumbing" (normalization + residuals) around them, covered below.',
      },
      ffnCard: {
        structureLabel: 'Structure',
        structureBody: 'Just two fully-connected layers and a non-linear activation. The first layer expands the vector from d to 4d ("unfold") → activation filters → the second layer compresses back to d ("contract"). Simple structure, but accounts for 2/3+ of model parameters.',
        mainstreamBadge: 'Mainstream interpretation',
        debateBadge: '⚠️ Still actively debated',
        mainstreamBody: 'A widely accepted interpretation (Geva 2021 et al.): each neuron in FFN\'s middle layer is like a "knowledge switch" — facts like "Paris is the capital of France" are encoded in these switches. Attention lets tokens talk to each other; FFN lets each token "look up" its own knowledge.',
        caveat: '※ Mainstream but contested — some research suggests FFN mainly does "feature integration", with knowledge distributed across multiple components. In rigorous contexts, FFN\'s exact role remains an open question.',
      },
      normResidual: {
        layerNormBadge: 'Normalization',
        layerNormTitle: 'LayerNorm',
        layerNormWhatLabel: 'What it is',
        layerNormWhatBody: 'Standardizes vector values into a stable range — neither too large nor too small.',
        layerNormWhyLabel: 'Why it\'s needed',
        layerNormWhyBody: 'During training, if a layer\'s outputs "drift" (become very large or very small), training becomes unstable. Normalization forces values back to a fixed range, letting training converge stably and enabling deep networks.',
        layerNormImportant: '📌 This is the engineering prerequisite for Transformers stacking 50+ layers.',
        layerNormCaveat: '※ Math formulas (μ / σ / γ / β) are in deep dive mode.',
        residualBadge: 'Residual connection',
        residualTitle: 'Residual',
        residualWhatLabel: 'What it is',
        residualWhatBody: 'Adds the input x of this layer directly (a "skip connection") onto the output of the main operation (Attention or FFN). A simple plus sign — y = F(x) + x.',
        residualWhyLabel: 'Why it\'s needed (analogy)',
        residualWhyBody1: 'Imagine cooking a complex dish: each preparation step might go wrong or lose the original flavor of an ingredient. A residual connection is like keeping a portion of the original at each step — even if the new step fails, the original flavor is preserved.',
        residualWhyBody2: 'Same applies to deep networks: when stacking 50+ layers, even if a middle layer "fails", the original information still flows through the residual channel — this is the key to reliably stacking very deep networks.',
        residualImportant: '📌 Without residuals, networks beyond a few layers simply won\'t train.',
      },
      multiHead: {
        heads: [
          { focus: 'Coreference', example: '"她" → "Lucy"' },
          { focus: 'Syntax', example: '"放" → subject + object' },
          { focus: 'Sentiment', example: '"累" = negative' },
          { focus: 'Long-range link', example: '"看到" → "猫"' },
        ],
        headFocusPrefix: 'Focus:',
        concatLabel: 'Concat (heads\' outputs joined into one long vector)',
        woLabel: 'W_O projection',
        woSub: 'Long vector → back to d dims',
        outputLabel: 'Final output vector',
      },
      ffnFlow: {
        tokenPrompt: 'Pick a token to see which "features" FFN activates:',
        inputLabel: 'Input',
        inputSublabel: '{token} vector (d={d}-dim sample)',
        midLabel: 'Middle layer',
        midSublabel: '{mid} dims · {active} features activated',
        outputLabel: 'Output',
        outputSublabel: 'New d-dim vector',
        w1Label: 'W₁ expand',
        w1Sub: 'd → 4d ({mid})',
        w2Label: 'W₂ contract',
        w2Sub: '4d → d',
        cellsLabel: '{n}-dim vector',
        dimTooltip: 'Dim {n}: {value}',
        insight: '💡 Switching tokens, watch which mid-layer features activate — they\'re different per token. This is the source of the "knowledge base lookup" analogy. Each mid-layer neuron is like a "knowledge switch", firing only when the input vector matches some learned knowledge. "巴黎" might activate "capital/France/Europe" switches; "苹果" might activate "fruit/tech company/red".',
        caveat: '※ Illustrative intuition. The ReLU activation in the middle layer crushes irrelevant knowledge to 0 and keeps relevant knowledge — that\'s the core role of non-linear activation.',
      },
      qkvDemo: {
        ch21Title: '2.1 · Core idea',
        ch21Body1: 'Attention boils down to one thing: ',
        ch21Body1Em: 'compute a "who to look at" weight per token, and use those weights to fuse the information of other tokens into itself.',
        ch21Body2: 'This means when the model processes "她", it can dynamically decide whether to look back at "Lucy", "Mary", or something else — not the fixed previous 3 tokens, but by content relevance. This is the core advantage of Transformer over RNN/CNN.',
        ch22Title: '2.2 · Using a search-engine analogy for Q / K / V / O',
        ch22Intro: 'What attention does is essentially what a search engine does: use a query to match against indices, retrieve content, and lay out results. These four matrices are the four components of LLM\'s "search engine":',
        matrices: [
          { name: 'Query · what I\'m looking for', analogy: 'The search term you type', llm: 'What the current token is "asking" — e.g., "她" asking "which earlier female name am I referring to?"' },
          { name: 'Key · how I can be found', analogy: '"Index tags" on every database entry — telling the engine "I\'m of type X"', llm: 'Each candidate token\'s "tag" — e.g., "Lucy" tagged as "person / female"' },
          { name: 'Value · the actual content', analogy: 'The actual page content you see when clicking through', llm: "Each token's semantic content — pulled out and fused after Q-K matches" },
          { name: 'Output · final assembly', analogy: 'How the search engine "lays out" the found results into the page you see', llm: 'Concatenate the multi-head outputs and project back to original dim (only multi-head attention needs O; single-head doesn\'t)' },
        ],
        matrixSearchEngineLabel: 'Search engine',
        matrixInLLMLabel: 'In LLM',
        matricesCaveat: '※ All four matrices are learned by the model during training. The full math is in deep dive mode.',
        ch23Title: '2.3 · Walk through a concrete example (click steps to see the diagram update)',
        ch23Body: 'Sentence: "Lucy 把书放在桌上，因为她累了". Attention is processing 她 — it needs to find which earlier token 她 refers to. Click the 4 steps below to see how the SVG changes.',
        ch24Title: '2.4 · Multi-head attention + W_O',
        ch24Body: 'In 2.3 we walked through one set of Q/K/V — that\'s one head, looking at context from one perspective. Real models use N heads in parallel (8–128 depending on model scale; GPT-3 uses 96), each watching a different relationship:',
        annotMultiHead: '🎭 Multi-head parallel',
        annotMultiHeadBody: 'N independent sets of Wq/Wk/Wv each do their own attention, fully independent.',
        annotConcat: '🪡 Concat',
        annotConcatBody: 'The N head outputs are concatenated head-to-tail into a long vector.',
        annotWO: '📐 W_O integration',
        annotWOBody: 'Multiply the long vector by W_O, compress back to d dims → final output.',
        paramHint: '💡 Multi-Head Attention has 4 matrix types: Wq, Wk, Wv (N copies each) + one W_O. This layer\'s parameter count is roughly 4 × d² — more parameters = slower inference and more VRAM.',
      },
      qkvWalkthrough: {
        steps: [
          { label: '① Compute Q', explainer: '"她" vector × Wq → Q vector. You can think of it as asking "which earlier female name am I referring to?"' },
          { label: '② Each token computes K', explainer: 'Every token uses Wk to compute its own K (its "tag"): "Lucy → person+female", "书 → object", "桌上 → location", ...' },
          { label: '③ Q · K = match scores', explainer: 'Q dot-product with each K, then softmax → a 0-1 probability per token. Below, line thickness = match score. "Lucy" 65%, "累" 10%, the rest much lower.' },
          { label: '④ Weighted sum of V', explainer: 'Sum each token\'s V (semantic content) weighted by the match score from step 3. The new vector for "她" = 0.65·V(Lucy) + 0.10·V(累) + ... → carries 65% of Lucy\'s information.' },
        ],
        qBubbleQ: 'Q',
        qBubbleHint: '"who, female?"',
        kPrefix: 'K:',
        kLabels: ['person+female', 'particle', 'object', 'verb', 'location', 'conjunction', 'pronoun', 'state', 'particle'],
        outputBoxLine1: '"她" output ≈',
        outputBoxLine2: '0.65·V(Lucy) + 0.10·V(累) + ...',
        prevStep: '← Previous',
        nextStep: 'Next →',
        progress: '{current} / {total}',
        caveatStart: '※ Full math (with softmax / scaling) is in ',
        caveatLink: 'deep dive mode',
        caveatEnd: ' — for everyday understanding, the "ask / find / fetch" four-step intuition is enough.',
      },
      attentionMatrix: {
        layerLabels: ['Layer 1 (neighbors)', 'Layer 2 (short phrases)', 'Layer 3 (long-range meaning)'],
        examplePickerLabel: 'Pick a representative sentence',
        examplePickerHintPrefix: '📌 Core point of this example:',
        examplePickerHintEnd: '(most visible at layer 3 — layers 1 and 2 still attend to neighbors)',
        layerPickerLabel: 'Switch layers',
        mainHeading: 'Hover over a token to see what it "looks at" (line thickness = attention weight)',
        legendFrom: 'Top row = currently hovered token (From)',
        legendTo: 'Bottom row = candidate targets (To, darker = stronger attention)',
        currentSelected: '💡 Currently selected: ',
        currentPositionPart: '(position {n}) at ',
        currentLayerSuffix: ' attention distribution — its top 3 attended tokens:',
        showMatrix: '▸ Show full N×N attention matrix values',
        crossLayerInsight: '🎯 Notice what happens when you switch layers: layer 1 mostly looks at neighboring tokens (syntax / local modifiers); by layer 3 attention spans long distances, establishing semantic links like "她 → Lucy" or "看到 → 猫". This is the concrete demonstration of "stacking layers lets the model abstract progressively".',
        caveat: '※ A reasonable teaching-purpose mock, not real GPT attention weights. Real models have 32+ heads per layer with different weights per head; this shows a simplified "multi-head averaged" version.',
      },
      blockDiagram: {
        normStyleLabel: 'Normalization position (affects data flow order)',
        preNormTitle: 'Pre-Norm (modern mainstream)',
        preNormSub: 'LN first, then Attention/FFN — LLaMA, GPT, Qwen, etc.',
        postNormTitle: 'Post-Norm (original)',
        postNormSub: 'Attention/FFN first, then LN — Vaswani 2017 original Transformer',
        introBold: 'Block',
        introMid: 'in full — the model has N layers (24–120), each one repeats this flow:',
        flowHeading: 'Full data flow ({mode})',
        nodeInput: 'Input x',
        nodeOutput: 'Output',
        nodeLayerNorm: 'LayerNorm',
        nodeAttention: 'Attention',
        nodeFFN: 'FFN',
        nodeAdd: '⊕ Add',
        nodeAddAttnSubPre: 'Residual: x + Attn(LN(x))',
        nodeAddFFNSubPre: 'Residual: x\' + FFN(LN(x\'))',
        nodeAddAttnSubPost: 'x + Attn(x)',
        nodeAddFFNSubPost: 'x\' + FFN(x\')',
        residualLabel: 'residual',
        summary1: '🧱 Every main operation (Attention / FFN) is wrapped in normalization + residual — keeps training stable and information flowing.',
        summary2: '※ Pre-Norm is the current mainstream (more stable training); Post-Norm is the original (theoretically slightly better but harder to train). See deep dive for details.',
      },
    },
    deepDive: {
...
const zh = {
  ...
  transformer: {
    ...
    comp: {
      overview: {
        boxInput: '输入',
        boxInputSub: '每个 token 的向量',
        boxAttn: 'Attention',
        boxAttnSub: '让 token 互相沟通',
        boxAttnExample: '"她" 看向 "Lucy"',
        boxFFN: 'FFN（前馈网络）',
        boxFFNSub: '每个 token 自己消化',
        boxFFNExample: '提取相关知识 / 特征',
        boxOutput: '输出',
        boxOutputSub: '下一层用',
        step1Label: '第 1 步',
        step1Title: 'Attention（注意力机制）',
        step1Sub: '让 token 互相沟通',
        step1Body: '一个 token 不能独自理解自己——"她" 这个字本身没意义，必须看回前文知道"她"指的是 Lucy 还是 Mary。Attention 让每个 token 主动看向句子里其他相关的 token，把上下文信息融合进自己的向量。',
        step1Hint: '※ 把它想成"开会"——每个 token 都在跟别的 token 交换情报，更新自己的"理解"。',
        step2Label: '第 2 步',
        step2Title: 'FFN（前馈网络）',
        step2Sub: '让每个 token 自己消化',
        step2Body: 'Attention 之后，每个 token 已经"听到"了其他 token 的信息。FFN 让每个 token 单独做一遍"思考"——把刚才听来的信息跟它本身携带的知识结合，提取出更高层的特征。',
        step2Hint: '※ 把它想成"散会后回办公室翻自己的脑子"——消化刚才的会议内容。',
        stackHint: '🔁 这两件事是堆叠的——大型 LLM 有几十到上百层，每一层都是「Attention → FFN」，逐层抽象出更复杂的语义。实际工程上还会在外面包两层"工程支持"（归一化 + 残差连接），下文会专讲。',
      },
      ffnCard: {
        structureLabel: '结构',
        structureBody: '就两层全连接 + 一个非线性激活函数。第一层把向量从 d 维放大到 4d 维（"展开"）→ 激活筛选 → 第二层缩回 d 维（"收回"）。结构简单，但参数量占整个模型 2/3+。',
        mainstreamBadge: '主流解释',
        debateBadge: '⚠️ 但学界仍在讨论',
        mainstreamBody: '一种被广泛接受的解释（Geva 2021 等）：FFN 的中间层每个神经元像一个"知识开关"——比如"巴黎是法国首都"这类事实知识编码在这些开关里。Attention 让 token 之间沟通，FFN 让每个 token 单独"查表"。',
        caveat: '※ 这是主流但有争议的解释——也有研究认为 FFN 主要做"特征整合"，知识可能分散在多个组件里。严谨场景下应该说"FFN 的具体作用仍是开放问题"。',
      },
      normResidual: {
        layerNormBadge: '归一化',
        layerNormTitle: 'LayerNorm',
        layerNormWhatLabel: '是什么',
        layerNormWhatBody: '把向量的数值"标准化"到一个稳定范围里——既不会过大也不会过小。',
        layerNormWhyLabel: '为什么需要',
        layerNormWhyBody: '神经网络训练时如果某层输出数值"飘"了（变得很大或很小），训练就不稳定了。归一化把数值强制拉回固定范围，让训练能稳定收敛、能堆深层网络。',
        layerNormImportant: '📌 这是 Transformer 能堆 50+ 层的工程前提。',
        layerNormCaveat: '※ 数学公式（μ / σ / γ / β）放在深入模式。',
        residualBadge: '残差连接',
        residualTitle: 'Residual',
        residualWhatLabel: '是什么',
        residualWhatBody: '把进入这一层之前的输入 x 直接"绕过去"加到主操作（Attention 或 FFN）的输出上。一个简单的"加号"——y = F(x) + x。',
        residualWhyLabel: '为什么需要（用一个比喻）',
        residualWhyBody1: '想象一下你做一道复杂菜：每加工一步都有可能出错或弄丢原料的味道。"残差连接"就像每一步都留一份原汁——即使新一步没做好，原汁还能保住基本味道。',
        residualWhyBody2: '对深层网络也一样：50+ 层堆叠时，哪怕中间某层"翻车"，原始信息还能通过残差通道直传过去——这是模型能可靠地堆很深的关键。',
        residualImportant: '📌 没有残差，超过几层的网络就训不起来。',
      },
      multiHead: {
        heads: [
          { focus: '指代关系', example: '"她"→"Lucy"' },
          { focus: '语法成分', example: '"放"→主语+宾语' },
          { focus: '情感倾向', example: '"累" = 负面' },
          { focus: '长距离关联', example: '"看到"→"猫"' },
        ],
        headFocusPrefix: '看',
        concatLabel: 'Concat（4 个 head 输出首尾拼成长向量）',
        woLabel: 'W_O 投影',
        woSub: '长向量 → 整理回 d 维',
        outputLabel: '最终输出向量',
      },
      ffnFlow: {
        tokenPrompt: '看不同 token 经过 FFN 激活了什么"特征"：',
        inputLabel: '输入',
        inputSublabel: '{token} 的向量（d={d} 维示意）',
        midLabel: '中间层',
        midSublabel: '{mid} 维 · {active} 个特征激活',
        outputLabel: '输出',
        outputSublabel: '新的 d 维向量',
        w1Label: 'W₁ 扩展',
        w1Sub: 'd → 4d ({mid})',
        w2Label: 'W₂ 收回',
        w2Sub: '4d → d',
        cellsLabel: '{n} 维向量',
        dimTooltip: '第 {n} 维：{value}',
        insight: '💡 切换不同 token 看中间层激活的差异：每个 token 激活的"特征"位置不一样——这就是"知识库查询"类比的来源。中间层每个神经元像一个"知识开关"，只在输入向量匹配它学到的某个知识时才被激活。例如"巴黎"可能激活"首都/法国/欧洲"开关；"苹果"可能激活"水果/科技公司/红色"等不同的开关。',
        caveat: '※ 这是直觉示意。中间层的 ReLU 激活让"无关知识"被压成 0、"相关知识"留下——这是非线性激活的核心作用。',
      },
      qkvDemo: {
        ch21Title: '2.1 · 核心思想',
        ch21Body1: 'Attention 的本质就一件事：',
        ch21Body1Em: '给每个 token 算一组「该看谁」的权重，按权重把其他 token 的信息加权融合进自己。',
        ch21Body2: '这意味着模型在处理"她"这个 token 时，能动态决定该看回 "Lucy"、"Mary" 还是别的——不是固定看前 3 个 token，而是按内容相关性。这是 Transformer 比 RNN/CNN 强的核心。',
        ch22Title: '2.2 · 用搜索引擎类比 Q / K / V / O 四个矩阵',
        ch22Intro: 'Attention 跟搜索引擎做的事其实一样：用查询词去匹配索引、取出内容、整理结果。这四个矩阵是 LLM 里"搜索引擎"的四个组件：',
        matrices: [
          { name: 'Query · 我在找什么', analogy: '你在搜索框里输入的查询词', llm: '当前 token 想"问"的问题——比如"她"问"我前面提到的女性人名是谁？"' },
          { name: 'Key · 我能被怎么找到', analogy: '数据库里每个条目的"索引标签"——告诉搜索引擎"我是什么类型"', llm: '每个候选 token 的"标签"——比如"Lucy"标记着"人名 / 女性"' },
          { name: 'Value · 实际内容是什么', analogy: '搜索结果点进去的"实际页面内容"', llm: '每个 token 的"语义信息"——Q-K 匹配后，从匹配的 token 取出来融合' },
          { name: 'Output · 最终输出整理', analogy: '搜索引擎把找到的结果"排版"成你看到的页面', llm: '把多头的结果拼接 + 投影回原维度（多头 attention 才需要 O；单头不需要）' },
        ],
        matrixSearchEngineLabel: '搜索引擎',
        matrixInLLMLabel: '在 LLM 里',
        matricesCaveat: '※ 这四个矩阵都是模型训练学出来的。具体的数学公式在深入模式里。',
        ch23Title: '2.3 · 一个例子走一遍完整流程（点 step 看图）',
        ch23Body: '例句："Lucy 把书放在桌上，因为她累了"。Attention 现在处理「她」——它要找出前面哪个 token 是它指的人。点下面 4 个 step 看 SVG 怎么变。',
        ch24Title: '2.4 · 多头注意力 + W_O 的角色',
        ch24Body: '前面 2.3 讲的一组 Q/K/V = 一个头（head）——只能从一个角度看上下文。实际模型用 N 个头并行（按模型规模 8 ~ 128 不等，GPT-3 用 96 头），每个头关注不同关系：',
        annotMultiHead: '🎭 多头并行',
        annotMultiHeadBody: 'N 套独立的 Wq/Wk/Wv 各做各的 attention，互不干扰。',
        annotConcat: '🪡 Concat 拼接',
        annotConcatBody: 'N 个头的输出首尾拼成一个长向量。',
        annotWO: '📐 W_O 整理',
        annotWOBody: '长向量乘以 W_O，缩回原 d 维 → 最终输出。',
        paramHint: '💡 完整 Multi-Head Attention 共 4 种矩阵：Wq、Wk、Wv 各 N 套 + 一个 W_O。这一层的参数量大约是 4 × d²——参数越多，推理越慢、占显存越多。',
      },
      qkvWalkthrough: {
        steps: [
          { label: '① 算出 Q', explainer: '"她" 的向量乘以 Wq → 得到 Q 向量。可以理解成它在"问"：「前面提到的女性人名是谁？」' },
          { label: '② 每个 token 算 K', explainer: '所有 token 都用 Wk 算出自己的 K（"标签"）：「Lucy → 人名+女」、「书 → 物品」、「桌上 → 地点」...' },
          { label: '③ Q · K 算匹配度', explainer: 'Q 跟每个 K 做点积，过 softmax 得到 0-1 概率。看下面连线粗细 = 匹配度。「Lucy」65%、「累」10%、其他都很低。' },
          { label: '④ 按匹配度加权 V 求和', explainer: '把每个 token 的 V（语义内容）按上一步的匹配度加权求和。「她」的新向量 = 0.65×V(Lucy) + 0.10×V(累) + ... → 携带 65% Lucy 的信息。' },
        ],
        qBubbleQ: 'Q',
        qBubbleHint: '"前面女性?"',
        kPrefix: 'K:',
        kLabels: ['人名+女', '助词', '物品', '动词', '地点', '连词', '代词', '状态', '助词'],
        outputBoxLine1: '「她」的输出 ≈',
        outputBoxLine2: '0.65·V(Lucy) + 0.10·V(累) + ...',
        prevStep: '← 上一步',
        nextStep: '下一步 →',
        progress: '{current} / {total}',
        caveatStart: '※ 完整数学公式（带 softmax / 缩放等）放在',
        caveatLink: '深入模式',
        caveatEnd: '里——日常理解用上面的"问 / 找 / 取"四步直觉就够。',
      },
      attentionMatrix: {
        layerLabels: ['第 1 层（看邻居）', '第 2 层（看短词组）', '第 3 层（跨距离理解）'],
        examplePickerLabel: '选个有代表性的句子',
        examplePickerHintPrefix: '📌 这个例子的核心看点：',
        examplePickerHintEnd: '（切到第 3 层时最明显——前 1、2 层 attention 还在看邻居）',
        layerPickerLabel: '切换到第几层看看',
        mainHeading: '鼠标悬停某个 token，看它"看向"哪里（连线粗细 = 注意力权重）',
        legendFrom: '上排 = 当前悬停的 token（From）',
        legendTo: '下排 = 候选目标（To，颜色越深 = 注意力越强）',
        currentSelected: '💡 当前选中：',
        currentPositionPart: '（位置 {n}）在',
        currentLayerSuffix: '的注意力分布——它最关注的 3 个 token：',
        showMatrix: '▸ 看完整 N×N 注意力矩阵数值',
        crossLayerInsight: '🎯 注意切换层会发生什么：第 1 层主要看邻近 token（句法 / 局部修饰）；到第 3 层 attention 会跨过长距离，建立"她 → Lucy"、"看到 → 猫"等语义关联。这就是"多层堆叠让模型逐层抽象"的具象演示。',
        caveat: '※ 这是教学演示用的合理近似 mock，不是真实 GPT 输出的注意力权重。真实模型每层有 32+ 个头，每个头的权重不同；这里展示的是"多头平均后"的简化版。',
      },
      blockDiagram: {
        normStyleLabel: '归一化位置（影响数据流顺序）',
        preNormTitle: 'Pre-Norm（现代主流）',
        preNormSub: '先 LN 再 Attention/FFN——LLaMA、GPT、Qwen 等',
        postNormTitle: 'Post-Norm（原始版）',
        postNormSub: '先 Attention/FFN 再 LN——Vaswani 2017 原始 Transformer',
        introBold: 'Block',
        introMid: '完整长这样——模型有 N 层（24-120），每层都重复这个流程：',
        flowHeading: '完整数据流（{mode}）',
        nodeInput: '输入 x',
        nodeOutput: '输出',
        nodeLayerNorm: 'LayerNorm',
        nodeAttention: 'Attention',
        nodeFFN: 'FFN',
        nodeAdd: '⊕ 加法',
        nodeAddAttnSubPre: '残差: x + Attn(LN(x))',
        nodeAddFFNSubPre: '残差: x\' + FFN(LN(x\'))',
        nodeAddAttnSubPost: 'x + Attn(x)',
        nodeAddFFNSubPost: 'x\' + FFN(x\')',
        residualLabel: '残差',
        summary1: '🧱 每个"主操作"（Attention / FFN）外面都包了归一化 + 残差——结构上保证训练稳定 + 信息畅通。',
        summary2: '※ Pre-Norm 是当前主流（训练更稳）；Post-Norm 是原始版（理论上效果略好但难训）。详见深入模式。',
      },
    },
    deepDive: {
